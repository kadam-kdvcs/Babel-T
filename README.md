...
在阶段 3.2 基础上接入 **SQLite 持久化、历史记录、人工译文中校意见**：
...
- 新增：历史记录查询、人工最终译文编辑、全文/分段审校意见保存
...
- 数据库文件默认：`data/translations.db`
- 可通过环境变量 `DATABASE_PATH` 指定其他路径
- 启动页面或第一次运行会自动建表
- 保存：文档、段落、运行状态、四种译文、人工译文、全文/分段审校意见
...
│   └── storage.py              # SQLite 存储层（文档/运行/结果/意见）
├── prompts/
...
- **阶段 4（当前）**：SQLite 保存审校记录，老师审校意见输入与保存
