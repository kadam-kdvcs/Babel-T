...
本阶段在阶段 2（阿里云机器翻译双模式）之上接入 **LLM 审校 + 纠正后译文**（DeepSeek OpenAI 兼容接口，双模式）：
...
- 显示：翻译对照（原始 API 译文 / LLM 纠正后译文）、术语命中表、专名命中表、审校报告
...
| `api` | 调用 DeepSeek（OpenAI 兼容接口）生成逐段纠正后译文 + 8 项结构审校报告 | 真实使用 |

**api 模式未配置密钥时自动回退占位报告并在页面黄色提示**；LLM 调用失败（网络/业务/解析错误）页面显示错误、保留上次成功结果（**api 翻译结果不丢**）。审校报告由 LLM 生成，仅供人工复核参考。LLM 是校准辅助：翻译 API 无法接入术语表（阿里云无 context 参数），术语一致性由 LLM 依据术语库把关；同时 LLM 会逐段给出**纠正后译文**，供页面两种展示模式切换。

**页面「翻译结果展示」radio**：可切换「显示 LLM 纠正过后的翻译」/「显示原始 API 翻译 + LLM 纠正过后的翻译结果（两者都显示）」两种视图——只影响展示，翻译与审校照常执行（api 翻译是不可省略的步骤）。审校报告始终保留在下方作为人工复核辅助。
...
- **阶段 3.1（当前）**：接入 LLM 纠正后译文 + 审校报告（DeepSeek OpenAI 兼容接口，双模式 mock/api，一次调用输出逐段纠正译文与 8 项报告结构，Key 走 `.env`）
