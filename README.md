...
本阶段在阶段 2（阿里云机器翻译双模式）之上接入 **LLM 多轮翻译结果 + 最终仲裁**（DeepSeek OpenAI 兼容接口，双模式）：
...
- 显示：四结果对比（原始 API 译文 / LLM 直接翻译 / LLM 修正结果 / LLM 最终仲裁结果）、术语命中表、专名命中表、审校报告
...
| `api` | 调用 DeepSeek（OpenAI 兼容接口）分三次独立调用：直接翻译 → 修正 → 最终仲裁+审校 | 真实使用 |

**api 模式未配置密钥时自动回退占位结果并在页面黄色提示**；LLM 调用失败（网络/业务/解析错误）页面显示错误、保留上次成功结果（**api 翻译结果不丢**）。LLM 会执行三轮处理：①直接翻译原文；②基于“原文 + API 译文”修正；③以原文为最高依据仲裁并产出最终结果。最终结果绝对不添加原文未有的信息。

**页面结果展示**：固定显示**四结果对比**——原始 API 译文、LLM 直接翻译结果、LLM 修正结果、LLM 最终仲裁结果；审校报告始终保留在下方作为人工复核辅助。

**性能与动态展示**：api 翻译多段文本时按段落并发请求（默认 4 线程），不必等所有段落串行完成；页面会**立即显示每段阿语原文**，译文位置显示「等待翻译…」，某段完成立刻在该段原文下方显示译文；全部翻译完成后再进入 LLM 纠正与审校阶段。

**本阶段明确不做**：SQLite 存储（阶段 4）、登录权限、部署、PDF/Word、词边界判断（纯子串会命中派生词，如「فلسطين」会命中「الفلسطينية」，属已知取舍）、术语约束发送给阿里云（该 API 无 context 参数，约束文本仅生成，供 LLM 审校使用）、让 LLM 脱离原文另译全文、让 LLM 添加原文没有的信息。
...
│   └── deploy_guide.md         # 临时部署指南（局域网 / 公网隧道供异地审查）
└── tests/                      # pytest 单元测试
```
...
- **阶段 3.2（当前）**：接入 LLM 直接翻译 / 修正 / 最终仲裁三种结果 + 8 项审校报告（DeepSeek OpenAI 兼容接口，双模式 mock/api，三次独立调用，各阶段只提供必要输入，页面四结果对比，Key 走 `.env`）
