# -*- coding: utf-8 -*-
"""翻译与审校配置中心（阶段 2 重构：配置集中管理；阶段 3 扩展 LLM 审校配置）。

模块作用
--------
集中管理翻译与 LLM 审校相关的全部配置：环境变量名、引擎标识、默认值、
配置对象（TranslationConfig / ReviewConfig）与配置加载函数
（load_translation_config / load_review_config）。
...
- reviewer.py 只保留审校逻辑（模板加载、请求构造、响应解析）。

职责约定
--------
- 本模块是翻译与审校相关配置的**唯一读取 os.environ 的地方**；
  translator.py / reviewer.py 只通过参数接收配置，保持纯函数特性、可测试；
- 模块内不 import python-dotenv：.env 的加载只发生在 app.py 顶部，
  命令行直接调用 run_pipeline 时需要自行 load_dotenv；
- 异常体系（TranslationError 家族）**刻意留在 translator.py**、审校异常
  （ReviewError 家族）**刻意留在 reviewer.py**：错误是各行为的对外契约
  （app.py 用 except translator.TranslationError / reviewer.ReviewError
  捕获），配置模块不混杂错误类型。
...
import os  # 标准库：读取环境变量（翻译与审校配置的唯一读取处）
...
# REVIEW_* 是 LLM 审校引擎通用配置；DEEPSEEK_* 是 DeepSeek 凭证与地址。
# DeepSeek 接口兼容 OpenAI 格式，其他同格式服务可通过 URL/模型名覆盖。
ENV_REVIEW_ENGINE = "REVIEW_ENGINE"                    # 审校引擎模式：mock / api
ENV_DEEPSEEK_API_KEY = "DEEPSEEK_API_KEY"              # LLM 审校 API 密钥（api 必填）
ENV_DEEPSEEK_API_URL = "DEEPSEEK_API_URL"              # LLM 接口 base URL（自动拼 /chat/completions）
ENV_REVIEW_MODEL = "REVIEW_MODEL"                      # LLM 审校模型名
ENV_REVIEW_TIMEOUT_SECONDS = "REVIEW_TIMEOUT_SECONDS"  # 单次审校请求超时（秒）
...
# 引擎模式标识（TRANSLATION_ENGINE 与 REVIEW_ENGINE 共用的取值）
MOCK_ENGINE = "mock"  # mock：占位（译文/报告），不联网（默认）
API_ENGINE = "api"    # api：调用真实接口（阿里云翻译 / DeepSeek 审校）
...
# LLM 审校相关默认值（load_review_config 在未设置/留空时使用；
# 审校引擎的默认值复用上面的 MOCK_ENGINE 常量，不另设 DEFAULT）
DEFAULT_DEEPSEEK_API_URL = "https://api.deepseek.com"  # DeepSeek base URL（无尾斜杠）
DEFAULT_REVIEW_MODEL = "deepseek-chat"                 # 默认 LLM 审校模型
DEFAULT_REVIEW_TIMEOUT = 30                            # 单次审校请求超时（秒）
...
class ReviewConfig:
    ...
    作用：把散落的 LLM 审校相关环境变量统一封装成一个对象，作为参数
          传给 reviewer 模块的函数，使其保持纯函数特性（不直接读环境变量）。
    输入：engine —— "mock" 或 "api"；api_key —— LLM 审校凭证（可能为空串）；
          api_url —— LLM 接口 base URL；model —— LLM 模型名；
          timeout_seconds —— 单次请求超时（秒）。
    输出：ReviewConfig 实例；frozen=True 表示实例不可修改
          （防止流水线中途被意外改掉配置）。
    """
    ...
    @property
    def has_credentials(self) -> bool:
        """是否已配置审校凭证（api_key 非空）。

        作用：api 模式缺 key 时即视为「无凭证」，回退占位报告。
        输入：无（读自身字段）。
        输出：bool —— api_key 非空为 True。
        """
...
def load_translation_config() -> TranslationConfig:
    ...
    作用：读取 8 个 TRANSLATION_*/ALIYUN_* 环境变量并做校验与默认值
          ...
          DEEPSEEK_API_KEY 不在此读取（LLM 审校凭证，由
          load_review_config 读取，见下方）。
...
def load_review_config() -> ReviewConfig:
    """从环境变量加载审校配置（审校配置的唯一入口，阶段 3 新增）。

    作用：读取 5 个 REVIEW_*/DEEPSEEK_* 环境变量并做校验与默认值
          填充，返回配置对象。所有值读取后 .strip()（防手误带空格）。
          与 load_translation_config 完全对称：
          - engine 空串 → 默认 mock；取值非法抛 ValueError；
          - timeout 非正整数抛 ValueError；空串 → 默认超时；
          - api_url / model 空串 → 默认值；
          - api_key 刻意不回落——空串正是「缺凭证 → 回退占位报告」
            的判定依据（与 AK/SK 同策略）。
    输入：无（隐式读取 os.environ）。
    输出：ReviewConfig —— 配置对象；engine 取值非法或
          timeout 非正整数时抛出 ValueError（中文消息）。
    """
    # os.environ.get(key, default)：读环境变量，未设置时返回 default。
    # .strip()：去掉首尾空白；「空串 = 未设置 = 用默认值」约定
    # 与 load_translation_config 一致（详见该函数内的注释）。
    engine_raw = os.environ.get(ENV_REVIEW_ENGINE, "").strip()
    engine = engine_raw or MOCK_ENGINE  # 空串（strip 后）→ 默认 mock
    # 引擎取值只允许 mock / api 两种，非法值立即抛错（配置错误，页面提示）
    if engine not in (MOCK_ENGINE, API_ENGINE):
        raise ValueError(
            f"环境变量 {ENV_REVIEW_ENGINE} 取值非法：{engine!r}，应为 {MOCK_ENGINE!r} 或 {API_ENGINE!r}"
        )

    # 超时必须是正整数：先尝试转 int，转不了或 ≤0 都算配置错误；
    # 空串同 engine 处理——视为未设置，用默认超时
    timeout_raw = os.environ.get(ENV_REVIEW_TIMEOUT_SECONDS, "").strip()
    timeout_raw = timeout_raw or str(DEFAULT_REVIEW_TIMEOUT)
    try:
        # int(str)：字符串转整数；转换失败会抛 ValueError
        timeout_seconds = int(timeout_raw)
    except ValueError:
        raise ValueError(
            f"环境变量 {ENV_REVIEW_TIMEOUT_SECONDS} 不是整数：{timeout_raw!r}"
        ) from None
    if timeout_seconds <= 0:
        raise ValueError(
            f"环境变量 {ENV_REVIEW_TIMEOUT_SECONDS} 必须为正整数"
        )

    return ReviewConfig(
        engine=engine,
        # api_key 刻意不回落：空串正是「缺凭证 → 回退占位」的判定依据
        # （与 load_translation_config 中 AK/SK 的处理策略一致）
        api_key=os.environ.get(ENV_DEEPSEEK_API_KEY, "").strip(),
        # 其余字段同 engine 约定：空串视为未设置，回落默认值
        api_url=os.environ.get(ENV_DEEPSEEK_API_URL, "").strip() or DEFAULT_DEEPSEEK_API_URL,
        model=os.environ.get(ENV_REVIEW_MODEL, "").strip() or DEFAULT_REVIEW_MODEL,
        timeout_seconds=timeout_seconds,
    )
